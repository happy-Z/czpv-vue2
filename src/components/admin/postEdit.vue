  ...
  export default {
    ...
    methods: {
      ...
      saveMinio(pos, fd) {
        ...
        this.$http.get(this.$constant.baseURL + "/minio/getUploadUrl", {key: key}, true)
          .then((res) => {
            if (!this.$common.isEmpty(res.data)) {
              // 使用预签名 URL 直接上传文件
              const xhr = new XMLHttpRequest();
              xhr.open('PUT', res.data, true);
              // 清空所有认证头，预签名 URL 已包含签名信息
              xhr.withCredentials = false;
              xhr.onload = () => {
                if (xhr.status === 200) {
                  let url = this.$store.state.sysConfig['minio.downloadUrl'] + key;
                  // 移除预签名 URL 中的查询参数（如果有的话）
                  if (url.includes('?')) {
                    url = url.split('?')[0];
                  }
                  this.$common.saveResource(this, "articlePicture", url, file.size, file.type, file.name, "minio", true);
                  this.$refs.md.$img2Url(pos, url);
                }
              };
              xhr.onerror = () => {
