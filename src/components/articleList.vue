    ...
    <div class="recent-post-item shadow-box background-opacity"
      ...
      <div class="recent-post-item-post" :class="{ leftImage: index % 2 === 0, rightImage: index % 2 !== 0 }">
        ...
        <el-tooltip placement="top" effect="light">
          ...
          <h3 v-html="renderTitle(article)"></h3>
        </el-tooltip>
        ...
        <!-- 内容 -->
        <div class="recent-post-desc" v-html="renderContent(article)">
        </div>
        <div class="search-hit-meta" v-if="showSearchMeta(article)">
          <span v-if="!$common.isEmpty(getScore(article))">相关度 {{ formatScore(article) }}</span>
          <span v-if="!$common.isEmpty(normalizedSearchKeyword)">命中词 {{ normalizedSearchKeyword }}</span>
        </div>
        <!-- 分类 标签 -->
  ...
  export default {
    ...
    computed: {
      normalizedSearchKeyword() {
        if (this.$common.isEmpty(this.searchContext)) {
          return "";
        }
        const raw = this.searchContext.keyword || this.searchContext.queryText || "";
        return String(raw).trim();
      },
      highlightTokens() {
        return this.extractKeywords(this.normalizedSearchKeyword);
      }
    },
    methods: {
      extractKeywords(text) {
        if (this.$common.isEmpty(text)) {
          return [];
        }
        const tokens = text
          .replace(/[+\-()"]/g, " ")
      ...
      renderHighlighted(text) {
        ...
        if (this.$common.isEmpty(safeText) || this.$common.isEmpty(this.highlightTokens)) {
          return safeText;
        }

        const pattern = this.highlightTokens
          .sort((a, b) => b.length - a.length)
          .map(item => this.escapeRegExp(item))
          .join("|");
        if (this.$common.isEmpty(pattern)) {
          return safeText;
        }

        const reg = new RegExp(`(${pattern})`, "ig");
        return safeText.replace(reg, "<mark class=\"es-hit\">$1</mark>");
      },
      normalizeHighlightValue(value) {
        if (this.$common.isEmpty(value) && value !== 0) {
          return "";
        }
        if (Array.isArray(value)) {
          return value
            .filter(item => !this.$common.isEmpty(item) || item === 0)
            .map(item => String(item))
            .join(" ... ");
        }
        return String(value);
      },
      resolveHighlight(article, field) {
        if (this.$common.isEmpty(article) || this.$common.isEmpty(article.highlights)) {
          return "";
        }
        return this.normalizeHighlightValue(article.highlights[field]);
      },
      renderTitle(article) {
        const highlight = this.resolveHighlight(article, "articleTitle");
        if (!this.$common.isEmpty(highlight)) {
          // 为ES返回的高亮添加特殊标记
          return `<span class="search-context-highlight">${highlight}</span>`;
        }
        return this.renderHighlighted(this.$common.isEmpty(article) ? "" : article.articleTitle);
      },
      renderContent(article) {
        const highlight = this.resolveHighlight(article, "articleContent");
        if (!this.$common.isEmpty(highlight)) {
          // 为ES返回的高亮添加特殊标记
          return `<span class="search-context-highlight">${highlight}</span>`;
        }
        return this.renderHighlighted(this.$common.isEmpty(article) ? "" : article.articleContent);
      },
      getScore(article) {
        if (this.$common.isEmpty(article)) {
          return null;
        }
        if (!this.$common.isEmpty(article._score) || article._score === 0) {
          return article._score;
        }
        if (!this.$common.isEmpty(article.score) || article.score === 0) {
          return article.score;
        }
        if (!this.$common.isEmpty(article.esScore) || article.esScore === 0) {
          return article.esScore;
        }
        return null;
      },
      formatScore(article) {
        const score = this.getScore(article);
        if (this.$common.isEmpty(score) && score !== 0) {
          return "";
        }
      ...
      showSearchMeta(article) {
        return !this.$common.isEmpty(this.normalizedSearchKeyword) || !this.$common.isEmpty(this.getScore(article));
      }
    }
  }
</script>
...
<style scoped>
  ...
  .recent-post-item:hover {
    ...
    box-shadow: 0 4px 16px rgba(81, 196, 146, 0.15);
  }
  ...
  .recent-post-desc::after {
    ...
    width: 100%;
    height: 24px;
    background: linear-gradient(to bottom, transparent, var(--descGradientColor, rgba(255, 255, 255, 0.9)));
    pointer-events: none;
  }
  ...
  .search-hit-meta {
    ...
    margin-top: 10px;
    padding-top: 8px;
    border-top: 1px dashed rgba(81, 196, 146, 0.3);
    display: flex;
    flex-wrap: wrap;
    gap: 12px;
  }
  ...
  .search-hit-meta span:hover {
    background: rgba(81, 196, 146, 0.15);
    color: var(--lightGreen);
  }

  /* 搜索高亮样式优化 */
  .recent-post-item-post >>> mark.es-hit {
    background: linear-gradient(120deg, rgba(239, 121, 79, 0.3), rgba(255, 201, 79, 0.2));
    color: #d84315;
    border-radius: 3px;
    padding: 1px 4px;
    font-weight: 500;
    box-shadow: 0 0 8px rgba(239, 121, 79, 0.2);
  }

  .recent-post-item-post >>> em {
    background: linear-gradient(120deg, rgba(239, 121, 79, 0.3), rgba(255, 201, 79, 0.2));
    color: #d84315;
    border-radius: 3px;
    padding: 1px 4px;
  ...
  /* ES返回的高亮标记 */
  .recent-post-item-post >>> .search-context-highlight {
    background: linear-gradient(120deg, rgba(81, 196, 146, 0.15), rgba(81, 196, 146, 0.05));
  }

  .recent-post-item-post >>> .search-context-highlight mark {
    background: linear-gradient(120deg, rgba(239, 121, 79, 0.3), rgba(255, 201, 79, 0.2));
    color: #d84315;
    border-radius: 3px;
    padding: 1px 4px;
  ...
  .recent-post-item-post >>> .search-context-highlight em {
    background: linear-gradient(120deg, rgba(239, 121, 79, 0.3), rgba(255, 201, 79, 0.2));
    color: #d84315;
    border-radius: 3px;
    padding: 1px 4px;
  ...
  @media screen and (max-width: 700px) {
    ...
    .recent-post-item:hover {
      border-left-color: var(--lightGreen);
      box-shadow: 0 2px 8px rgba(81, 196, 146, 0.1);
    }
