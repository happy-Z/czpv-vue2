  ...
  export default {
    ...
    methods: {
      ...
      customUpload(options) {
        const storeType = (this.storeType || localStorage.getItem("defaultStoreType") || "local").trim();
        const fileName = (options.file && options.file.name) ? options.file.name : ("upload_" + new Date().getTime());
        let suffix = "";
        if (fileName.lastIndexOf('.') !== -1) {
          suffix = fileName.substring(fileName.lastIndexOf('.'));
        }

        const current = !this.$common.isEmpty(this.$store.state.currentUser) ? this.$store.state.currentUser : this.$store.state.currentAdmin;
        const username = (current && current.username ? current.username.replace(/[^a-zA-Z]/g, '') : "") || "user";
        const userId = (current && (current.id || current.userId)) ? (current.id || current.userId) : "";
        let key = this.prefix + "/" + username + userId + new Date().getTime() + Math.floor(Math.random() * 1000) + suffix;
        ...
        if (storeType === "local") {
          ...
          const xhr = new XMLHttpRequest();
          xhr.open('get', this.$constant.baseURL + "/minio/getUploadUrl?key=" + key, false);
          if (this.isAdmin) {
            xhr.setRequestHeader("Authorization", localStorage.getItem("adminToken"));
          } else {
            xhr.setRequestHeader("Authorization", localStorage.getItem("userToken"));
          ...
          try {
            ...
            if (res !== null && res.hasOwnProperty("code") && res.code === 200) {
              ...
              // 直接用 PUT 方法上传到预签名 URL，不使用 FormData
              return new Promise((resolve, reject) => {
                ...
                uploadXhr.onerror = () => options.onError(uploadXhr.response);
                uploadXhr.onload = () => {
                  if (uploadXhr.status >= 200 && uploadXhr.status < 300) {
                    options.onSuccess({ url: uploadUrl });
                  } else {
                    options.onError(uploadXhr.response);
                  }
                };
                uploadXhr.open('put', uploadUrl);
                uploadXhr.send(options.file);
              });
            } else if (res !== null && res.hasOwnProperty("code") && res.code !== 200) {
              return Promise.reject(res.message);
            } else {
        ...
        return Promise.reject("未识别的存储方式: " + storeType);
      },
      ...
      handleSuccess(response, file, fileList) {
        ...
        if (this.storeType === "local") {
          if (typeof response === "string") {
            ...
            url = (response && (response.data || response.url || (response.data && response.data.url) || (response.data && response.data.data))) || "";
          }
        ...
        } else if (this.storeType === "minio") {
          url = (response && (response.data || response.url)) || (this.$store.state.sysConfig['minio.downloadUrl'] + this.prefix + "/" + file.name);
          // 移除预签名 URL 中的查询参数
          if (url.includes('?')) {
            url = url.split('?')[0];
          }
          this.$common.saveResource(this, this.prefix, url, file.size, file.raw.type, file.name, "minio", this.isAdmin);
        }
        url = (url || "").toString().trim();
        if (this.$common.isEmpty(url)) {
          this.$message({
            message: "上传成功但未拿到图片地址，请重试",
            type: "error"
          });
          return;
        }
